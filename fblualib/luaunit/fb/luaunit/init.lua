...
function errormsg(actual, expected)
    local errorMsg
    if USE_EXPECTED_ACTUAL_IN_ASSERT_EQUALS then
...
FBJsonOutput = {
  __class__ = 'FBJsonOutput',
  runner = nil,
  result = nil,
}
FBJsonOutput_MT = { __index = FBJsonOutput }

function FBJsonOutput:new()
  local t = {}
  setmetatable(t, FBJsonOutput_MT)
  return t
end
...
LuaUnit_MT = { __index = LuaUnit }
    ...
    function LuaUnit:setOutputType(outputType)
        ...
        if outputType:upper() == "FB_JSON" then
            self.outputType = FBJsonOutput
            return
        end
    ...
    function LuaUnit:runSomeTest( someName, someInstance )
        ...
        if (string.sub(someName, 1, string.len(self.TEST_FUNCTION_PREFIX)+1) ==
            self.TEST_FUNCTION_PREFIX .. ':') then
          someName = string.sub(someName,
                                string.len(self.TEST_FUNCTION_PREFIX) + 2)
        else
      ...
      for key, val in pairs(_G) do
        if string.sub(key,1,4):lower() == 'test' then
          if type(val) == 'function' then
            table.insert(tests, self.TEST_FUNCTION_PREFIX .. ':' .. key)
          elseif type(val) == 'table' then
            for methodName, methodInstance in pairs(val) do
              if (type(methodInstance) == 'function' and
                  string.sub(methodName,1,4) == 'test') then
                table.insert(tests, key .. ':' .. methodName)
              end
            end
          end
        end
